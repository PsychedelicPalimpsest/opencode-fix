...
describe("tool.registry", () => {
  ...
  it.instance("exposes bash and bash_background as separate tools", () =>
    Effect.gen(function* () {
      const registry = yield* ToolRegistry.Service
      const agent = yield* Agent.Service
      const build = yield* agent.get("build")
      ...
      const bashBackground = tools.find((tool) => tool.id === "bash_background")
      expect(bash).toBeDefined()
      expect(bashBackground).toBeDefined()

      // Both tools should be distinct
      expect(bash?.id).not.toBe(bashBackground?.id)
    }),
  )
